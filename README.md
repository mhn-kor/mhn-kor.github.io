...
- 목록은 최신 300개만 불러옵니다 (`app.js`의 `LIMIT`). 300건이면 응답 27KB.
- **무한스크롤은 받아오기가 아니라 그리기만 나눕니다** (`PAGE = 20`, PC 5열 기준 4줄).
  체크 여부가 기기 localStorage 에만 있어 서버는 무엇이 미체크인지 모릅니다. 서버에서 20개씩
  끊어 오면 앞쪽이 전부 체크된 재방문자는 빈 화면을 받고, 스크롤할 내용이 없어 다음 요청이
  영영 걸리지 않습니다. 필터를 먼저 걸고 자르는 순서도 같은 이유로 중요합니다.
- 그래서 목록 요청은 방문당 1회입니다. 27KB × 무료 플랜 5GB = 약 18만 회분이라 여유가 큽니다.
