...
네이버TV 는 **세로 숏폼이 `/h/<번호>`, 가로가 `/v/<번호>` 인데 번호 공간은 하나**입니다.
둘 다 받아서 `/v/` 로 모읍니다 — 같은 영상이 두 주소로 각각 올라오는 것을 `unique` 로 막아야 하고,
`/v/` 는 세로 숏폼도 그대로 재생합니다(반대로 `/h/` 는 가로 영상을 `/v/` 로 되돌립니다).
임베드는 어느 쪽이든 `/embed/<번호>` 하나입니다.
...
**여기 올라오는 영상은 전부 세로입니다** — 몬헌나우가 세로 화면 게임이라 가로 토벌 영상이
나올 수가 없습니다. 그래서 크게 보기 상자는 유튜브 숏츠·치지직 클립·네이버TV 가 **다 같은 9:16**
입니다(`.rk-v-body.yt, .cz, .nv` 한 줄).

임베드 셋 다 `<video>` 가 iframe 을 그대로 채우고 `object-fit: contain` 으로 그립니다
(조상 어디에도 고정 비율이 없는 것을 확인했습니다). 그래서 **상자 비율이 곧 영상 크기**입니다 —
9:16 을 주면 세로 영상이 꽉 차고, 상자가 납작할수록 그만큼 letterbox 되어 작아집니다.
치지직·네이버TV 를 4:3 이나 16:9 로 뒀다가 영상이 작아진 이력이 있습니다.
