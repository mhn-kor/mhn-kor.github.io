...
유튜브(숏츠 · 일반 · youtu.be) · X(트위터) · 틱톡 주소를 받아
**아래 세 형태 중 하나로 정규화해서 저장**합니다.
...
**X 와 틱톡은 쓸 수 있는 썸네일 주소가 없습니다.** X 의 syndication 은 CORS 로 막히고,
틱톡 oEmbed 는 CORS 는 열려 있지만 주소에 만료 서명(`x-expires`)이 붙어 담아 둘 수가 없습니다.
빈 검은 칸은 고장 난 것처럼 보이므로 카드에 그 서비스 로고를 옅게 깝니다(`.rk-logo`).
