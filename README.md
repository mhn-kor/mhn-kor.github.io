...
## 3. 이벤트 탭 — 디스코드 웹훅 (한 번만)

이벤트 탭의 «참여하기» 는 응모를 **디스코드 채팅방으로 바로 보냅니다.** 사이트에도,
Supabase 에도 아무것도 남지 않습니다 — 이미지까지 그대로 흘려보냅니다.

**웹훅 주소를 `app.js` 나 `event.js` 에 넣지 마세요.** 이 저장소의 스크립트는 전부
공개라, 넣는 순간 누구나 그 주소로 채팅방에 아무거나 쏠 수 있고 막을 방법이
웹훅 삭제밖에 없습니다. 주소를 아는 곳은 Edge Function 하나뿐이어야 합니다.

```bash
# 1) 디스코드에서 웹훅 만들기
#    채널 우클릭 → 채널 편집 → 연동 → 웹후크 → 새 웹후크 → URL 복사

# 2) Supabase CLI 로 주소를 «시크릿» 으로 넣습니다 (저장소에 남지 않습니다)
supabase login
supabase link --project-ref frmpcahnrclrzfkaerbw
...
JWT 가 아닙니다. 검증을 켜 두면 anon 키를 보내도 파싱에 실패해 응모가 전부 거부됩니다.
(대신 함수 안에서 값 검사와 IP 당 횟수 제한을 합니다.)

- 웹훅을 바꾸려면 `supabase secrets set` 을 다시 하면 됩니다. 앱 배포는 그대로 둡니다.
- **로컬 미리보기(docker compose)에는 함수가 없습니다.** 응모는 «같은 길» 을 그대로 타고
  404 에서 끝나며 «아직 배포되지 않았습니다» 라고 알려 줍니다. 로컬만 따로 빠져나가게
  만들면 그 길이 시험되지 않은 채 남아, 배포본에서 처음 도는 코드가 됩니다.
  보낼 주소도 로컬 API 로 잡아 두어, 로컬에서 실수로 «진짜» 채팅방에 쏘는 일이 없습니다.
- 보내는 동안 버튼이 «보내는 중…»(이미지가 있으면 «이미지 올리는 중…»)으로 바뀌고
...
(→ [마스터 비밀번호](#마스터-비밀번호-관리자-강제-삭제)). 탭의 «운영자 · 이벤트 열기»
버튼은 누구에게나 보이지만, 비밀번호가 틀리면 아무것도 만들어지지 않습니다.
`app_config` 에 `master_pw` 가 없으면 **아무도 이벤트를 못 엽니다**(fail closed).

응모 목록은 어디에도 없습니다. 당첨자 확인과 발표는 디스코드 채팅방에서 합니다.

### 기간

`starts_at` · `ends_at` 은 둘 다 비워 둘 수 있습니다.

| 시작 | 종료 | 결과 |
|---|---|---|
| 없음 | 없음 | **상시** — 지울 때까지 계속 받습니다 |
| 있음 | 없음 | 그때부터 계속 |
| 없음 | 있음 | 지금부터 그때까지 |
| 있음 | 있음 | 그 사이에만 |

목록은 **진행중 → 시작 전 → 선착순 마감 → 종료** 순으로 서고, 같은 무리 안에서는
**최근에 등록된 것이 위**입니다. 진행중이 아닌 이벤트는 참여 버튼이 잠깁니다.
끝난 이벤트가 지금 응모할 수 있는 것보다 위에 있으면 목록의 쓸모가 없습니다.

> **잠긴 버튼은 안내일 뿐입니다.** 함수 주소는 누구나 직접 부를 수 있고, 창을 띄워 둔
> 채로 이벤트가 끝나기도 합니다. 그래서 `discord-entry` 가 **서버 시계로** 기간을 다시
> 보고 `NOT_STARTED` · `ENDED` 로 거절합니다. 화면의 시계는 방문자 기기 것이라
> 배지가 잠깐 틀릴 수 있어도, 실제로 채팅방에 나가는 것은 서버가 정합니다.

**브라우저 달력 피커에는 «확인» 버튼이 없습니다.** 날짜를 고르면 창이 그냥 닫히고,
`datetime-local` 은 시각 칸이 비어 있는 동안 값이 통째로 «빈 값» 이라 **고르고 나서
아무 일도 안 일어난 것처럼 보였습니다.** 그래서 등록 창은

- **날짜칸과 시각칸을 나눴습니다.** `type=date` 는 날짜를 누르는 순간 값이 들어옵니다.
  달력의 «날짜 클릭» 이 곧 확인입니다.
- 시각은 비어 있으면 **시작 `00:00` · 종료 `23:59`** 로 알아서 채웁니다. 이벤트는
  «8월 4일부터 10일까지» 처럼 날짜로 말하지 시각으로 말하지 않습니다. 직접 고치면
  그쪽이 이깁니다(예: 오후 8시 시작).
- 고른 값을 바로 아래에서 **카드와 같은 문장으로 되읽어 줍니다**
  (`26. 8. 4. ~ 26. 8. 10. · 7일간`). 사람이 확인할 것은 버튼이 아니라 결과입니다.
  거꾸로 된 기간은 등록을 누르기 전에 빨갛게 알려줍니다.
- **오늘부터 1주일 · 1개월 · 비우기** 버튼으로 달력을 열지 않고도 채웁니다.
  «1주일» 은 오늘을 포함한 7일입니다(`+6일 23:59`).
- 화면에 그릴 때도 시각이 `00:00` · `23:59` 면 **날짜만** 보여 줍니다 —
  `26. 8. 4. 오전 12:00` 은 읽는 사람을 헷갈리게 하는 글자입니다.

기간을 나중에 고치려면 대시보드에서 그 행의 `starts_at` · `ends_at` 을 바꾸면 됩니다
(수정 UI 는 두지 않았습니다 — 지우고 다시 여는 편이 빠릅니다).

### 첫 화면 공지 창

진행중인 이벤트가 있으면 **사이트에 들어올 때 한 번** 창으로 알립니다. 이벤트 탭은
아홉 개 중 하나라, 알아서 눌러 주기를 기다리면 공지가 되지 않습니다.

- 진행중인 것이 여러 개면 **가장 최근에 등록된** 하나만 뜹니다.
  시작 전 · 선착순 마감 · 종료된 이벤트로는 뜨지 않습니다.
- 창 아래 **이벤트 바로가기** 가 이벤트 탭을 엽니다.
- **오늘 하루 안 보기** 를 누르면 그 이벤트가 24시간 동안 뜨지 않습니다.
  그냥 «닫기» 나 ESC 로 닫으면 다음 방문에 다시 뜹니다.
- **어느 탭에서 들어오든 뜹니다** — 공지사항 · 친구 코드 · 빌드 · 리더보드 어디서나.
  빌드·기록 공유 링크로 들어와도 뜹니다.
- 예외는 **이벤트 공유 링크(`?ev=…`)** 하나뿐입니다. 이미 그 이벤트를 보러 온 사람이고,
  대신 그 카드로 데려가 잠깐 짚어 줍니다.

> **덮어 두는 값에 이벤트 번호를 함께 저장합니다** (`localStorage` 의 `mhnkr.evseen`,
> `{ id, until }`). 시간만 저장하면 그 사이에 «새» 이벤트가 열려도 하루 동안 같이
> 묻힙니다 — 알리려고 만든 창이 새 소식을 가리게 됩니다. 번호가 다르면 기한이 남아
> 있어도 뜹니다.

이 창을 띄우려고 **어느 탭에서든 페이지를 열 때 이벤트 목록을 한 번 받아옵니다.**
그 목록은 이벤트 탭에서도 그대로 쓰므로 탭을 열 때 다시 받지 않습니다.

### 선착순

등록 창의 «선착순 인원» 을 채우면 그 인원까지만 받습니다. 비우면 무제한입니다.

**세는 방식이 이 기능의 전부입니다.** «지금 몇 명인지 읽고 → 자리가 있으면 → 한 명
늘린다» 로 만들면, 두 사람이 동시에 마지막 자리를 눌렀을 때 둘 다 «자리 있음» 을 읽고
둘 다 늘려 정원을 넘깁니다. 선착순은 정확히 그 순간을 위한 기능이라 그렇게 만들면 있으나
마나입니다. 그래서 검사와 증가를 SQL **한 문장** 안에 둡니다 (`claim_event_slot`):

```sql
update public.events set entries = entries + 1
 where id = p_id and (capacity is null or entries < capacity) and ...기간...
```

`update ... where` 는 행을 잠그고 조건을 다시 본 뒤에 씁니다. 실제로 남은 자리 6개에
동시 40건을 밀어넣어 **정확히 6건만** 통과하는 것을 확인했습니다.

- **`claim_event_slot` · `release_event_slot` 은 `service_role` 만 부를 수 있습니다.**
  anon 에게 열어 두면 응모하지도 않고 claim 만 반복해 30명짜리를 30번 요청으로 채워
  버릴 수 있습니다. Edge Function 은 `SUPABASE_SERVICE_ROLE_KEY` 로 부릅니다 —
  Supabase 가 자동으로 넣어 주므로 따로 설정할 것이 없습니다.
- 카드에는 **참여 인원**이 늘 보입니다 — 정원이 있으면 `참여 24 / 30명 · 남은 자리 6`,
  무제한이면 `참여 12명`. 끝났거나 아직인 이벤트는 «남은 자리» 만 뗍니다.
- 순서는 **자리 확보 → 디스코드 전송** 입니다. 반대로 하면 정원을 못 지킵니다.
  전송이 실패하면 잡아 둔 자리를 되돌립니다(`release_event_slot`).
- 로컬 미리보기에는 `service_role` 롤이 없어 `dev/03-seed.sql` 이 anon 에게 열어 줍니다.
  **그 두 줄은 로컬 전용입니다. `schema.sql` 로 옮기지 마세요.**

### 중복 응모 막기

**정원이 있는 이벤트는 한 닉네임이 한 번만** 응모합니다(`event_claims`). 정원이 없는
이벤트는 그대로 둡니다 — «표류연성 자랑» 처럼 여러 장 올릴 만한 것이 있고, 막아서
얻는 것도 없습니다.

이 표에 남는 것은 **닉네임을 이벤트 번호와 함께 해싱한 값 하나**뿐입니다. 제목도 내용도
이미지도 없고, 읽어도 누가 무엇을 냈는지 알 수 없습니다. 그래도 같은 닉네임이면 같은
값이 나오므로 두 번째를 막을 수 있습니다. 이벤트 번호를 섞는 이유는, 안 섞으면 한 사람의
값이 이벤트마다 같아 **이 표만으로 «누가 어느 이벤트에 참여했는지» 가 드러나기** 때문입니다.

순서는 **자리 확보 → 중복 표시** 입니다. 반대로 하면 자리가 없어 되돌아간 사람이 응모도
못 한 채 영영 막힙니다. 중복이면 방금 늘린 자리를 그 자리에서 되돌립니다. 디스코드 전송이
실패했을 때도 `release_event_slot` 이 자리와 중복 표시를 같이 지웁니다.

> **닉네임을 바꿔 다시 내면 막지 못합니다.** 로그인이 없으니 이 표로 풀 수 있는 문제가
> 아니고, 상품은 어차피 닉네임으로 나갑니다. 나머지는 IP 당 10분에 5건이 막습니다.

정원을 잘못 넣었거나 숫자가 어긋났으면 대시보드에서 그 행의 `entries` · `capacity` 를
직접 고치면 됩니다. 나중에 UI 로 늘리는 기능은 두지 않았습니다.

### 공유

카드마다 **링크 복사** 와 **카카오톡 공유** 가 있습니다. 주소는 `?ev=<id>#event` 로,
빌드(`?build=`) · 기록(`?rec=`) 과 같은 꼴입니다. 그 링크로 들어오면 이벤트 탭이 열리고
해당 카드로 스크롤해 **확실하게 짚어 줍니다 — 걷히지 않고 그대로 남습니다.**

- 카드 맨 위에 **«링크로 받은 이벤트입니다»** 띠를 박습니다.
- **나머지 카드를 한 톤 낮춰** 하나만 밝힙니다.
- 두꺼운 테두리를 두르고, 처음 세 번만 숨을 쉬어 시선을 끕니다.

테두리만으로는 부족합니다 — 진행중 카드는 원래 왼쪽에 호박색 띠를 두르고 있어 묻힙니다.
«이 카드가 그 이벤트입니다» 라고 글자로 말하는 것이 가장 확실합니다. 링크를 받은 사람은
목록 어디를 봐야 하는지 모르는 채로 들어오고, 몇 초 만에 사라지면 놓치면 그만입니다.
목록을 다시 그려도 살아 있습니다.

**마감·종료된 이벤트에는 공유 버튼이 없습니다.** 링크를 받은 사람이 할 수 있는 일이
없기 때문입니다. 시작 전 이벤트에는 남깁니다 — «이런 게 열린다» 를 미리 알리는 것이
공유의 쓸모입니다. 링크를 직접 열면 마감·종료된 이벤트도 정상적으로 짚어 줍니다.

카카오 SDK 준비와 링크 복사는 **빌드 탭 것을 그대로 씁니다**(`bdKakaoReady` · `bdCopyLink`).
같은 일을 두 벌 두면 키를 바꿀 때 한쪽만 낡습니다. 카카오 키가 없거나 PC 라면 기기 공유
시트 → 링크 복사 순으로 넘어갑니다(리더보드 공유와 동일).

### 확인

```bash
docker compose up -d
node tools/event-test.js            # 탭 · 목록 · 기간 · 선착순 · 공지 창 · 공유 · 운영자 등록/삭제
node tools/discord-entry-test.js    # 함수의 값 검사 · 자리 확보 · 임베드 (디스코드로 안 나갑니다)
```

선착순의 동시성은 DB 에서 직접 확인하는 편이 확실합니다:

```bash
# 남은 자리보다 훨씬 많이 동시에 밀어넣고, 정확히 남은 만큼만 통과하는지 봅니다.
node -e "
const A='http://localhost:8080/rest/v1';
(async()=>{
  const c=()=>fetch(A+'/rpc/claim_event_slot',{method:'POST',
    headers:{'Content-Type':'application/json',apikey:'x'},body:'{\"p_id\":1}'}).then(r=>r.json());
  const b=(await (await fetch(A+'/events?select=entries,capacity&id=eq.1')).json())[0];
  const r=await Promise.all(Array.from({length:40},c));
  const a=(await (await fetch(A+'/events?select=entries,capacity&id=eq.1')).json())[0];
  console.log('남은 자리',b.capacity-b.entries,'· 통과',r.filter(x=>x.ok).length,'· 끝',a.entries+'/'+a.capacity);
})()"
```
...
인스타는 릴스도 일반 글도 **같은 코드**입니다 — `/reel/` · `/reels/` · `/p/` 가 다 같은 글이라
셋 다 받아 `/reel/` 로 모읍니다. 공유 링크의 `?igsh=…` 는 쿼리라 저절로 떨어집니다.
...
(인스타 릴스는 «로그인 벽 때문에 안 된다» 고 적어 뒀다가 뒤집었습니다 — 실제로 열어 보니
로그인 벽도 없고 `<video>` 에 CDN 소스가 그대로 물립니다.)
...
**인스타만 예외로 7:10 입니다**(`.rk-v-body.ig`). 인스타 임베드는 영상칸을 **폭 × 1.25** 로 잡아
9:16 릴스도 4:5 로 자르고, 그 위에 프로필줄 55px 을 얹습니다. 그래서 세로로 길게 줄수록 폭이
좁아져 영상까지 같이 작아집니다 — `55 + 폭 × 1.25 = 높이` 인 7:10 이 잘리지 않으면서 가장 큰
지점입니다(9:16 이면 영상칸 266×333, 7:10 이면 330×413). 아래쪽 «더 보기»·아이콘 줄은
상자 밖으로 밀려 잘립니다.
...
`RK_SRC[].mark` 에 갈래마다 로고 SVG 가 한 벌씩 있습니다(치지직은 번개 «Z», 네이버는 «N» —
실제 로고 파일을 받아 좌표를 땄습니다). 유튜브는 썸네일이 있어 카드에는 안 쓰이지만
아래 등록창 로고 줄이 쓰므로 같이 들고 있습니다.

카드 오른쪽 위의 **갈래 배지(`.rk-src`)도 글자가 아니라 이 로고**입니다.
그림만 남으면 읽어주는 기기가 건너뛰므로 `role="img"` + `aria-label` 로 이름을 답니다
(마우스에는 `title`). 갈래를 못 알아본 카드만 «영상» 이라는 글이 섭니다.

**등록창의 «받는 곳» 줄도 이 표에서 그립니다**(`rkFormUI` → `#rk-url-srcs`).
이름을 다 적으면 좁은 화면에서 두 줄로 넘어가서 로고만 세웁니다 — 갈래를 늘리면 저절로 따라옵니다.
로고는 장식이라 읽어주는 기기에는 `aria-label` 로 이름을 글로 줍니다.
...
공유 URL(`?build=…#build`)도 같은 계열이었습니다 — `drawBuild()` 가 링크의 빌드 하나로
`bdState` 를 갈아끼워, 그 화면에서 아무 편집이든 저장되는 순간 내 빌드가 전부
지워졌습니다. 지금은 **저장값을 먼저 불러온 뒤 링크를 맨 앞에 새 카드로 얹습니다**
(링크를 연 목적이 그 빌드라 스크롤 없이 바로 보여야 합니다). 얹기만 하고
저장하지는 않습니다 — 리더보드 «빌드 보기» 처럼 구경만 하는 링크가 저장 목록에 쌓이면
안 되고, 편집을 시작하면 그 편집의 `bdSave` 가 링크 카드도 함께 저장합니다. 쓴 링크
값은 `replaceState` 로 주소에서 지워 새로고침 중복을 막습니다. 쿼리를 읽을 때 `+` 를
공백으로 되돌리는 이유는 `app.js` 의 `showTab()` 이 `searchParams` 를 만지며 쿼리를 폼
인코딩으로 다시 쓰기 때문입니다. `node tools/build-share-e2e.js` 가 이 한 바퀴(열기 →
재클릭 → 편집 → 새로고침, 깨진 링크·꽉 찬 목록 포함)를 브라우저로 돕니다.
...
> 그대로 두면 합계 막대가 `1/5` 가 아니라 칸 하나로 그려집니다.
> 그래서 `build.js` 의 `bdStoneLevels()` 가 **표류연성 데이터(`SMELT`)에서 레벨 수와 설명을
> 빌려 옵니다** — `bdTotalRow` 의 막대 칸과 `bdSkillDesc` 의 상세 수치 줄 둘 다 이 폴백을 탑니다.
> 생성기를 고쳐 넣는 방법도 있지만, 번들을 새로 받아야 해서 다른 데이터까지 같이 움직입니다.
> `build-test.js` 의 «표류석 스킬은 모두 상한과 설명을 얻는다» 가 이 폴백이 끊기면 잡습니다.

**합계에서 스킬 이름을 누르면** 레벨별 설명이 창으로 뜹니다(`bdOpenSkill`). 표류연성 탭과
같은 표(`.lv`)를 그대로 쓰되, **굵게 서는 줄이 «MAX» 가 아니라 «지금 이 빌드의 레벨»** 입니다
(`.lv.bd-lvcur` 가 `:last-child` 강조를 되돌리고 `.on` 줄로 옮깁니다). 상한을 넘겨 낀 빌드는
표에 없는 레벨이라 마지막 줄을 짚고 제목에 `/최대` 를 답니다.
설명이 있는 스킬만 `<button>` 이 되어 키보드로도 열립니다 — 없는 스킬은 눌러 봐야 빈 창입니다.
추천빌드 미리보기도 같은 합계 줄을 쓰므로 그 안에서도 눌립니다(창 위에 창이 겹쳐 열립니다).
...
- **삭제, 그리고 이벤트 열기에 씁니다.** 끌어올리기(`bump_friend_code`)에는 통하지 않습니다.
  이벤트 탭의 등록·삭제(`add_event` · `delete_event`)는 **오직** 이 값으로만 됩니다.
